...
async function getCurrentUser(req) {
  ...
  const [users] = await pool.execute(`
    ...
    FROM usuarios u JOIN perfiles p ON u.perfil_id = p.id LEFT JOIN empresas e ON e.id = u.empresa_id
...
router.get('/', async (req, res) => {
  try {
    ...
    const tenantFilter = user.es_superadmin ? '' : 'WHERE u.empresa_id = ?';
    const tenantParams = user.es_superadmin ? [] : [user.empresa_id];
    const [rows] = await pool.execute(`
      SELECT u.id, u.username, u.nombre_completo, u.correo, u.perfil_id, u.empresa_id,
             u.activo, u.ultimo_acceso, u.created_at,
             p.nombre as perfil_nombre, e.nombre as empresa_nombre
      FROM usuarios u JOIN perfiles p ON u.perfil_id = p.id LEFT JOIN empresas e ON e.id = u.empresa_id ${tenantFilter} ORDER BY u.id
    `, tenantParams);
    res.json(rows);
  } catch (error) {
    console.error(error);
...
router.post('/', async (req, res) => {
  try {
    ...
    const [result] = await pool.execute(
      'INSERT INTO usuarios (username, password_hash, password_salt, nombre_completo, correo, perfil_id, empresa_id, es_superadmin, activo) VALUES (?, ?, ?, ?, ?, ?, ?, 0, ?)',
      [username, hash, salt, nombre_completo, correo || '', perfil_id, user.es_superadmin ? (req.body.empresa_id || null) : user.empresa_id, activo !== undefined ? activo : 1]
    );
