...
router.get('/', async (req, res) => {
  try {
    const query = `
      ...
      WHERE (p.empresa_id = ? OR ? = 1)
      ORDER BY p.created_at DESC
    `;
    const [rows] = await pool.execute(query, [req.user.empresa_id, req.user.es_superadmin ? 1 : 0]);
    res.json(rows);
  } catch (error) {
...
router.post('/', async (req, res) => {
  try {
    ...
    const query = `
      INSERT INTO proyectos (nombre, cliente_id, cotizacion_id, estado_id, fecha_entrega_estimada, notas, empresa_id)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `;
    const values = [
      nombre, 
...
router.put('/:id', async (req, res) => {
  try {
    ...
    const values = [
      ...
      projectId, req.user.empresa_id, req.user.es_superadmin ? 1 : 0
    ];
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    const [result] = await pool.execute('DELETE FROM proyectos WHERE id = ? AND (empresa_id = ? OR ? = 1)', [projectId, req.user.empresa_id, req.user.es_superadmin ? 1 : 0]);
...
router.get('/:id/tareas', async (req, res) => {
  try {
    const projectId = req.params.id;
    const [rows] = await pool.execute('SELECT t.* FROM tareas_proyecto t JOIN proyectos p ON p.id = t.proyecto_id WHERE t.proyecto_id = ? AND (p.empresa_id = ? OR ? = 1) ORDER BY t.created_at ASC', [projectId, req.user.empresa_id, req.user.es_superadmin ? 1 : 0]);
    res.json(rows);
  } catch (error) {
