...
router.put('/:id', async (req, res) => {
  try {
    ...
    await pool.execute(
      ...
      [d.categoria !== undefined ? d.categoria : current.categoria,
       d.marca !== undefined ? d.marca : current.marca,
       d.modelo !== undefined ? d.modelo : current.modelo,
       d.descripcion !== undefined ? d.descripcion : current.descripcion,
       d.potencia_wp !== undefined ? d.potencia_wp : current.potencia_wp,
       d.potencia_kw !== undefined ? d.potencia_kw : current.potencia_kw,
       d.capacidad_kwh !== undefined ? d.capacidad_kwh : current.capacidad_kwh,
       d.tipo !== undefined ? d.tipo : current.tipo,
       d.costo !== undefined ? d.costo : current.costo,
       d.precio_venta !== undefined ? d.precio_venta : current.precio_venta,
       d.utilidad_pct !== undefined ? d.utilidad_pct : current.utilidad_pct,
       d.unidad !== undefined ? d.unidad : current.unidad,
       d.peso_kg !== undefined ? d.peso_kg : current.peso_kg,
       d.area_m2 !== undefined ? d.area_m2 : current.area_m2,
       d.activo !== undefined ? d.activo : current.activo,
       d.imagen_url !== undefined ? d.imagen_url : current.imagen_url,
       d.iva !== undefined ? d.iva : current.iva,
       req.params.id]
    );
...
router.post('/extract-data', upload.single('file'), async (req, res) => {
  try {
    ...
    if (req.file) {
      ...
      // Las fichas de producto suelen exponer información en metadatos,
      // tablas o JSON-LD aunque el contenido visible se cargue con JavaScript.
      const metadata = [
        $('title').first().text(),
    ...
    // Mantener la solicitud dentro del límite TPM de Groq y priorizar el contenido técnico.
    ...
    const prompt = `
Extract the technical specifications of a solar equipment (${categoria}) from the following webpage content.
...
Prioritize the product title, model, technical specification tables, JSON-LD data and metadata.
Use the product category to interpret equivalent labels in Spanish or English (for example kW, kWp, rated power, nominal power, output power).
Return numbers only for numeric fields, without units or thousands separators. Do not invent values.
...
${text}
    ...
    const hasTechnicalData = parsedData.marca || parsedData.modelo || parsedData.potencia_wp ||
      parsedData.potencia_kw || parsedData.capacidad_kwh || parsedData.peso_kg || parsedData.area_m2;
    if (!hasTechnicalData) {
      return res.status(422).json({ detail: 'La página no expone datos técnicos legibles. Prueba con la URL de la ficha técnica o sube el PDF del producto.' });
    }
