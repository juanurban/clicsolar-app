...
function normalizarTermino(t) {
    if (typeof t === 'string') {
        const texto = t;
        const soloSolar = /panel|bater[ií]a|generaci[oó]n|RETIE|operador de red|legalizaci[oó]n/i.test(texto);
        return { texto, aplica: soloSolar ? 'solar' : 'ambos' };
    }
    return { texto: t?.texto || '', aplica: ['ambos', 'solar', 'suministro'].includes(t?.aplica) ? t.aplica : 'ambos' };
}
...
window.renderTerminos = function() {
    ...
    list.innerHTML = '';
    terminosList = terminosList.map(normalizarTermino);
        ...
        list.innerHTML += `
            <div class="flex items-start gap-2 bg-surface-container rounded-lg p-3 group">
                ...
                <select class="sq-input w-44 text-xs" onchange="updateTerminoAplica(${i}, this.value)" title="Dónde aplica">
                    ...
                    <option value="solar" ${t.aplica === 'solar' ? 'selected' : ''}>Solo planta solar</option>
                    <option value="suministro" ${t.aplica === 'suministro' ? 'selected' : ''}>Solo suministro</option>
                </select>
...
window.addTermino = function() {
    terminosList.push({ texto: '', aplica: 'ambos' });
    renderTerminos();
}

window.updateTermino = function(index, value) {
    terminosList[index] = normalizarTermino(terminosList[index]);
    terminosList[index].texto = value;
    document.getElementById('inp-terminos_condiciones').value = JSON.stringify(terminosList);
}

window.updateTerminoAplica = function(index, value) {
    terminosList[index] = normalizarTermino(terminosList[index]);
    terminosList[index].aplica = value;
    document.getElementById('inp-terminos_condiciones').value = JSON.stringify(terminosList);
}
...
async function renderConfiguracion() {
    ...
    try {
        ...
        try {
            terminosList = JSON.parse(v('terminos_condiciones') || '[]').map(normalizarTermino);
                ...
                <form id="config-form" onsubmit="saveConfiguracion(event)">
                    ...
                    <div class="bg-surface-container-low p-8 lg:p-10 rounded-xl">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                <label class="sq-label">Correo Electrónico</label>
                                <input type="email" name="asesor_correo" class="sq-input" value="${v('asesor_correo')}" placeholder="asesor@plantassolaresdecolombia.com">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-surface-container-low p-8 lg:p-10 rounded-xl">
                        <div class="flex items-center gap-4 mb-8">
                            ...
                            <div class="flex-1">
                                ...
                                <p class="text-sm text-on-surface-variant">Estas condiciones aparecerán debajo de la firma. Selecciona si aplican a ambas modalidades, solo a plantas solares o solo a suministros.</p>
                            </div>
