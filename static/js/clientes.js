...
function cargarTerritoriosColombia() {
    ...
    return colombiaTerritoriosPromise;
}

async function initSelectoresUbicacion(cliente = {}) {
    const departamento = document.getElementById('inp-departamento');
    const municipio = document.getElementById('inp-municipio');
    if (!departamento || !municipio) return;
    try {
        const territorios = await cargarTerritoriosColombia();
        departamento.innerHTML = '<option value="">Selecciona un departamento</option>' +
            territorios.map(t => `<option value="${t.departamento}" ${t.departamento === (cliente.departamento || '') ? 'selected' : ''}>${t.departamento}</option>`).join('');
        const cargarMunicipios = (nombre, seleccionado = '') => {
            const territorio = territorios.find(t => t.departamento === nombre);
            municipio.disabled = !territorio;
            municipio.innerHTML = territorio
                ? '<option value="">Selecciona un municipio</option>' + territorio.ciudades.map(m => `<option value="${m}" ${m === seleccionado ? 'selected' : ''}>${m}</option>`).join('')
                : '<option value="">Selecciona primero un departamento</option>';
            document.getElementById('inp-ciudad').value = seleccionado || '';
        };
        cargarMunicipios(departamento.value, cliente.municipio || cliente.ciudad || '');
        departamento.onchange = () => {
            cargarMunicipios(departamento.value);
            document.getElementById('inp-hsp').value = '';
            document.getElementById('hsp-status').textContent = 'Selecciona un municipio para calcular la HSP';
        };
        municipio.onchange = calcularHspUbicacion;
    } catch (error) {
        showToast(error.message, 'error');
    }
}

async function calcularHspUbicacion() {
    const departamento = document.getElementById('inp-departamento')?.value;
    const municipio = document.getElementById('inp-municipio')?.value;
    const hsp = document.getElementById('inp-hsp');
    const status = document.getElementById('hsp-status');
    if (!departamento || !municipio || !hsp) return;
    document.getElementById('inp-ciudad').value = municipio;
    hsp.value = '';
    status.textContent = 'Calculando HSP con IA…';
    try {
        const result = await API.post('/cotizaciones/calcular-hsp', { departamento, municipio });
        hsp.value = Number(result.hsp).toFixed(2);
        status.textContent = `${result.fuente}: ${result.detalle || 'valor estimado para la ubicación'}`;
    } catch (error) {
        status.textContent = 'No se pudo calcular la HSP automáticamente';
        showToast(error.message, 'error');
    }
}
...
async function openClienteModal(id = null) {
    let c = {
        ...
        ciudad: '', departamento: '', municipio: '', operador_red: '', tipo_tarifa: 'Residencial',
                ...
                <div id="tab-perfil" class="hidden grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="md:col-span-2 bg-primary-container/10 border border-primary/30 rounded-xl p-5">
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                <h3 class="font-label-bold text-on-surface">Importar perfil del analizador</h3>
                                <p class="text-sm text-on-surface-variant mt-1">Sube un Excel horario para calcular la demanda, el consumo mensual estimado y el perfil horario del cliente.</p>
                            </div>
                        </div>
                        ${id ? `
                        ...
                        <div class="mt-5 pt-4 border-t border-primary/20">
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    <h3 class="font-label-bold text-on-surface">Crear perfil desde recibo</h3>
                                    <p class="text-sm text-on-surface-variant mt-1">Carga un recibo PDF digital para extraer consumo mensual, tarifa y valor facturado.</p>
                                </div>
                            </div>
