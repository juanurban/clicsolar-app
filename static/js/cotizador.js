    ...
    try {
        ...
        if (App.editCotizacionData) {
            ...
            if (Array.isArray(rawItems)) {
                ...
                stateCotizador.tipoPropuesta = rawItems.tipo_propuesta || 'solar';
...
function renderStep1(container) {
    ...
    if (stateCotizador.clienteSelected) {
        ...
        profileHtml = `
            ...
            <div class="mt-8 flex justify-end">
                <button onclick="goToStep(2)" class="sq-btn sq-btn-primary">${stateCotizador.tipoPropuesta === 'suministro' ? 'Continuar a Productos y Servicios' : 'Continuar al Diseño Técnico'} <span class="material-symbols-outlined">arrow_forward</span></button>
            </div>
        `;
    }
    ...
    container.innerHTML = `
        <div class="p-8 fade-in">
            ...
            <div class="mb-6">
                <label class="sq-label">Tipo de propuesta</label>
                <select id="tipo-propuesta" class="sq-input" onchange="cambiarTipoPropuesta(this.value)">
                    <option value="solar" ${stateCotizador.tipoPropuesta === 'solar' ? 'selected' : ''}>Planta solar fotovoltaica</option>
                    <option value="suministro" ${stateCotizador.tipoPropuesta === 'suministro' ? 'selected' : ''}>Suministro de productos y servicios</option>
                </select>
                <p class="text-xs text-on-surface-variant mt-2">Cotiza lámparas solares, postes, pedestales, instalación, mantenimiento y otros ítems del inventario.</p>
            </div>
...
function cambiarTipoPropuesta(tipo) {
    stateCotizador.tipoPropuesta = tipo === 'suministro' ? 'suministro' : 'solar';
    stateCotizador.dimensionamiento = null;
    stateCotizador.financiero = null;
    stateCotizador.items = [];
    stateCotizador.removedAutoItems = [];
    if (stateCotizador.tipoPropuesta === 'suministro' && stateCotizador.clienteSelected) {
...
async function renderStep2(container) {
    if (stateCotizador.tipoPropuesta === 'suministro') {
        renderStep2Suministro(container);
        return;
    }
        ...
        <div class="p-8 lg:p-10 fade-in flex flex-col gap-10">
            ...
            <div class="bg-surface-container-high rounded-xl overflow-hidden border border-outline-variant/20">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full min-w-[1000px] text-left table-fixed">
                        <thead class="text-xs uppercase text-on-surface-variant bg-surface-container border-b border-outline-variant/20">
                            <tr>
                                <th class="px-6 py-4 w-[175px]">Categoría</th>
                                <th class="px-6 py-4 w-[330px]">Ítem</th>
                        ...
                        <tbody>
                            ${(stateCotizador.items || []).map(it => {
                                ...
                                if (options && options.length > 0) {
                                    nombreHtml = `<select class="sq-input py-2 px-3 text-sm w-full max-w-none" title="Seleccionar ${it.nombre}" onchange="changeItemEquipo('${it.id}', this.value)">`;
                                    options.forEach(opt => {
...
function rebuildBOM() {
    ...
    const textoEquipo = (equipo) => `${equipo?.marca || ''} ${equipo?.modelo || ''}`.trim().toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
    const buscarEquipo = (lista, texto) => lista.find(e => textoEquipo(e).includes(texto)) || null;
    const equipoAutoActual = (id, lista) => {
        const item = (stateCotizador.items || []).find(i => i.id === id && i.isAuto);
        return item ? lista.find(e => e.id === item.equipo_id) || null : null;
    };
    const elegirEquipo = (id, lista, preferido, fallback) => equipoAutoActual(id, lista) || buscarEquipo(lista, preferido) || fallback;
    ...
    // 1. INVERSOR — plantilla predeterminada para nuevas cotizaciones
    const inversor = elegirEquipo('auto-inversor', stateCotizador.inversores, 'sun2000-10ktl-m1', d.inversor_sugerido);
    if (inversor) {
        d.inversor_sugerido = inversor;
        addAuto('auto-inversor', inversor, 1, 'inversor');
    }

    // 2. PANEL SOLAR
    const panel = elegirEquipo('auto-panel', stateCotizador.paneles, 'cs7n-730tb-ag', d.panel);
    if (panel) {
        d.panel = panel;
        addAuto('auto-panel', panel, d.num_paneles, 'panel');
    }

    // 3. BATERÍA
    const bat = elegirEquipo('auto-bateria', stateCotizador.baterias, 'fla48230', stateCotizador.baterias[0]);
    if (bat) {
        addAuto('auto-bateria', bat, 1, 'bateria');
    }

    // 4. SERVICIO DE INSTALACIÓN
    const mo = elegirEquipo('auto-mo', stateCotizador.servicios, 'servicio de instalacion', null);
    if (mo) {
        addAuto('auto-mo', mo, 1, 'servicio');
    }

    // 5. ACCESORIOS DE INSTALACIÓN
    const acc = elegirEquipo('auto-acc', stateCotizador.materiales, 'accesorios de instalacion', null);
    if (acc) {
        addAuto('auto-acc', acc, 1, 'estructura');
    }

    // 6. CERTIFICACIÓN RETIE
    const retie = elegirEquipo('auto-retie', stateCotizador.servicios, 'certificacion retie', null);
    if (retie) {
        addAuto('auto-retie', retie, 1, 'servicio');
    }

    // 7. DISEÑO E INGENIERÍA
    const dis = elegirEquipo('auto-dis', stateCotizador.servicios, 'diseno e ingenieria', null);
    if (dis) {
        addAuto('auto-dis', dis, 1, 'servicio');
    }

    // 8. TRÁMITE INCENTIVOS UPME
    const inc = elegirEquipo('auto-inc', stateCotizador.servicios, 'tramite incentivos upme', null);
    if (inc) {
        addAuto('auto-inc', inc, 1, 'servicio');
    }

    // 9. TRÁMITE OPERADOR DE RED
    const opRed = elegirEquipo('auto-opred', stateCotizador.servicios, 'tramite operador de red', null);
    if (opRed) {
        addAuto('auto-opred', opRed, 1, 'servicio');
    }

    // 10. TRANSPORTE DE EQUIPOS Y PERSONAL
    const trans = elegirEquipo('auto-trans', stateCotizador.servicios, 'transporte de equipos y personal', null);
    if (trans) {
...
function addExtraItem() {
    ...
    else if (type === 'prod') eq = stateCotizador.productos.find(e => e.id === eqId);
...
async function calculateFinances() {
    ...
    if (stateCotizador.tipoPropuesta === 'suministro') {
        ...
        stateCotizador.financiero = {
            ...
            ivaPct: 19, totalConIva, total_inversion: totalConIva,
            escenario_1: { titulo: 'Suministro de productos y servicios', ahorro_mensual: 0, ahorro_anual: 0, roi_meses: 0, roi_anos: 0 },
            escenario_2: { titulo: 'Suministro de productos y servicios', deduccion_renta: 0, beneficio_fiscal: 0, inversion_neta: totalConIva, ahorro_mensual: 0, ahorro_anual: 0, roi_meses: 0, roi_anos: 0 },
            resumen_25_anos: { inversion_inicial: totalConIva, ahorro_neto_25: 0, roi_total_pct: 0 },
            simulacion_excedentes: { precio_excedente_kwh: 0, kwh_autoconsumo_mes: 0, kwh_excedentes_mes: 0, ahorro_autoconsumo_mes: 0, ingreso_excedentes_mes: 0, beneficio_total_mes: 0, beneficio_total_anual: 0 },
            proyeccion: []
        };
        return;
    }
    ...
    const costoKwh = parseFloat(c.costo_kwh ?? c.cliente_costo_kwh) || 0;
    if (!conf.exc || conf.exc === 0) {
        conf.exc = Math.round(costoKwh * 0.30);
    }
...
function renderStep3Suministro(container) {
    const f = stateCotizador.financiero;
    const c = stateCotizador.config;
    container.innerHTML = `
        <div class="p-8 lg:p-10 fade-in flex flex-col gap-8">
            <div><h2 class="font-headline-md text-on-surface">Resumen financiero del suministro</h2><p class="text-on-surface-variant mt-2">Revisa el margen comercial, el IVA aplicable y el valor total de la propuesta.</p></div>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="bg-surface-container-low p-6 rounded-xl"><label class="sq-label">Margen comercial</label><input type="range" class="w-full accent-primary" min="0" max="50" step="1" value="${c.margen}" onchange="updateConfig('margen', this.value)"><div class="text-primary text-2xl font-label-bold mt-2">${c.margen}%</div></div>
                <div class="bg-surface-container-low p-6 rounded-xl"><div class="text-xs uppercase text-on-surface-variant">Subtotal con margen</div><div class="text-2xl font-label-bold text-on-surface mt-2">${formatCurrency(f.subtotalConMargen)}</div></div>
                <div class="bg-primary/10 p-6 rounded-xl border border-primary/30"><div class="text-xs uppercase text-on-surface-variant">Total de la propuesta</div><div class="text-3xl font-label-bold text-primary mt-2">${formatCurrency(f.total_inversion)}</div><div class="text-xs text-on-surface-variant mt-1">IVA incluido según los ítems marcados</div></div>
            </div>
            <div class="bg-surface-container-high rounded-xl p-6"><h3 class="font-headline-md text-on-surface mb-4">Detalle económico</h3><div class="flex justify-between border-b border-outline-variant/20 py-3"><span>Subtotal</span><strong>${formatCurrency(f.subtotal)}</strong></div><div class="flex justify-between border-b border-outline-variant/20 py-3"><span>Margen comercial</span><strong>${formatCurrency(f.montoMargen)}</strong></div><div class="flex justify-between border-b border-outline-variant/20 py-3"><span>IVA</span><strong>${formatCurrency(f.montoIva)}</strong></div><div class="flex justify-between pt-4 text-xl text-primary"><strong>TOTAL</strong><strong>${formatCurrency(f.total_inversion)}</strong></div></div>
            <div class="flex justify-between"><button onclick="goToStep(2)" class="sq-btn sq-btn-ghost">Atrás</button><button onclick="goToStep(4)" class="sq-btn sq-btn-primary">Generar Propuesta <span class="material-symbols-outlined">arrow_forward</span></button></div>
        </div>`;
}

function renderStep3(container) {
        if (stateCotizador.tipoPropuesta === 'suministro') {
            renderStep3Suministro(container);
            return;
        }
        const f = stateCotizador.financiero;
        const c = stateCotizador.config;
...
function renderStep4(container) {
    ...
    const d = stateCotizador.dimensionamiento;
    const f = stateCotizador.financiero;
    const esSuministro = stateCotizador.tipoPropuesta === 'suministro';
                ...
                <div class="flex-1">
                    <div class="bg-surface-container-high p-6 rounded-xl border border-primary/20 relative overflow-hidden mb-6">
                        ...
                        <div class="grid grid-cols-2 gap-y-4 gap-x-2 text-sm">
                            ...
                            <div class="text-on-surface-variant">Tipo:</div>
                            <div class="font-label-bold">${esSuministro ? 'Productos y servicios' : 'Planta solar fotovoltaica'}</div>
                            ...
                            ${esSuministro ? '' : `<div class="text-on-surface-variant">Sistema:</div><div class="font-label-bold">${d.potencia_kwp} kWp</div><div class="text-on-surface-variant">Retorno (Ley 1715):</div><div class="font-label-bold">${f.escenario_2.roi_anos} Años</div><div class="text-on-surface-variant">Ahorro 25 Años:</div><div class="font-label-bold text-success">+${formatCurrency(f.resumen_25_anos.ahorro_neto_25)}</div>`}
                        </div>
                    </div>
...
async function guardarCotizacion(estado, openPdf = false) {
    ...
    const payload = {
        ...
        items_json: JSON.stringify({
            tipo_propuesta: stateCotizador.tipoPropuesta,
